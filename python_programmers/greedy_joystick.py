...
def solution(name):
    ...
    for idx, char in enumerate(name):
        print('idx::', idx, 'char::', char, 'ord(char)::', ord(char), 'ord(A)::', ord('A'), 'ord(Z)::', ord('Z'), 'toA::',ord(char) - ord('A'), 'toZ::',ord('Z') - ord(char) + 1)

        # 위, 아래 조작 횟수의 최솟값 구하기
        answer += min(ord(char) - ord('A'), ord('Z') - ord(char) + 1)
        print('answer::',answer,'min::',min(ord(char) - ord('A'), ord('Z') - ord(char) + 1))
